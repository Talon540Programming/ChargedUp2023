...
public class VisionPoseEstimator {
    ...
    public VisionPoseEstimator(PhotonCamera camera, Transform3d robotToCamera, AprilTagFieldLayout fieldLayout) {
        m_camera = camera;
    ...
    public Optional<EstimatedRobotPose> getRobotPose() {
        ...
        if(0 < m_timestampCacheSeconds && Math.abs(m_timestampCacheSeconds - pipelineTimestamp) < 1e-6) {
            return Optional.empty();
        }
        ...
        if(pipelineResult.getTargets().size() < 2) {
            ...
            Optional<Pose3d> targetPosition = m_fieldLayout.getTagPose(lowestAmbiguityTarget.getFiducialId());

            if (targetPosition.isEmpty()) {
                DriverStation.reportWarning(
                        "[VisionPoseEstimator] The found tag was not within the FieldLayout.",
                        false
                );
                return Optional.empty();
            }
