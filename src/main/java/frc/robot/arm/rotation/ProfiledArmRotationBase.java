...
public class ProfiledArmRotationBase extends ProfiledPIDSubsystem {
  private final WPI_TalonFX rotationLeader =
      new WPI_TalonFX(
          HardwareDevices.Arm.Rotation.kArmRotationLeader.id,
          HardwareDevices.Arm.Rotation.kArmRotationLeader.controller);
  private final WPI_TalonFX rotationFollower =
      new WPI_TalonFX(
          HardwareDevices.Arm.Rotation.kArmRotationFollower.id,
          HardwareDevices.Arm.Rotation.kArmRotationFollower.controller);

  private final WPI_CANCoder rotationEncoder =
      new WPI_CANCoder(
          HardwareDevices.Arm.Rotation.kArmRotationEncoder.id,
          HardwareDevices.Arm.Rotation.kArmRotationEncoder.controller);

  private final DigitalInput forwardBeamBreak =
      new DigitalInput(HardwareDevices.Arm.Rotation.kForwardBeamBreakPort);
  private final DigitalInput rearBeamBreak =
      new DigitalInput(HardwareDevices.Arm.Rotation.kRearBeamBreakPort);

  private final ArmFeedforward m_feedForward =
      new ArmFeedforward(ControlValues.kS, ControlValues.kG, ControlValues.kV, ControlValues.kA);
