...
public class AutoFactory {
    ...
    public AutoFactory(DriveBase driveBase, ArmBase armBase, IntakeBase intakeBase) {
        ...
        m_eventMap = Map.ofEntries(
            // Intake Commands
            Map.entry("StartIntakeIn", Commands.runOnce(() -> m_intakeBase.setVoltage(8), m_intakeBase)),
            Map.entry("StartIntakeOut", Commands.runOnce(() -> m_intakeBase.setVoltage(-8), m_intakeBase)),
            ...
            Map.entry("StartArmFloor", Commands.runOnce(() -> m_armBase.updateState(ArmState.FLOOR), m_armBase)),
            Map.entry("StartArmHybrid", Commands.runOnce(() -> m_armBase.updateState(ArmState.SCORE_HYBRID), m_armBase)),
            Map.entry("StartArmConeMid", Commands.runOnce(() -> m_armBase.updateState(ArmState.SCORE_MID_CONE), m_armBase)),
            Map.entry("StartArmCubeMid", Commands.runOnce(() -> m_armBase.updateState(ArmState.SCORE_MID_CUBE), m_armBase)),
            Map.entry("StartArmCubeHigh", Commands.runOnce(() -> m_armBase.updateState(ArmState.SCORE_HIGH_CUBE), m_armBase)),
            Map.entry("StartArmFloorInvert", Commands.runOnce(() -> m_armBase.updateState(ArmState.FLOOR.invert()), m_armBase)),
            Map.entry("StartArmHybridInvert", Commands.runOnce(() -> m_armBase.updateState(ArmState.SCORE_HYBRID.invert()), m_armBase)),
            Map.entry("StartArmConeMidInvert", Commands.runOnce(() -> m_armBase.updateState(ArmState.SCORE_MID_CONE.invert()), m_armBase)),
            Map.entry("StartArmCubeMidInvert", Commands.runOnce(() -> m_armBase.updateState(ArmState.SCORE_MID_CUBE.invert()), m_armBase)),
            Map.entry("StartArmCubeHighInvert", Commands.runOnce(() -> m_armBase.updateState(ArmState.SCORE_HIGH_CUBE.invert()), m_armBase)),
            Map.entry("StartArmIdle", Commands.runOnce(() -> m_armBase.updateState(ArmState.FLOOR), m_armBase)),
            ...
            Map.entry("GoToConeMidInvert", new GoToState(m_armBase, ArmState.SCORE_MID_CONE.invert())),
            Map.entry("GoToCubeMidInvert", new GoToState(m_armBase, ArmState.SCORE_MID_CUBE.invert())),
            Map.entry("GoToCubeHighInvert", new GoToState(m_armBase, ArmState.SCORE_HIGH_CUBE.invert())),
    ...
    public CommandBase createAutoCommandFromPaths(List<PathPlannerTrajectory> paths) {
        ...
        return autoBuilder
                .fullAuto(paths)
                .beforeStarting(Commands.either(Commands.none(), new CalibrateArmExtension(m_armBase), m_armBase::extensionCalibrated));
    }
