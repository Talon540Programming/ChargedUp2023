...
public class IntakeStateController extends CommandBase {
  ...
  public void execute() {
    ...
    m_wristPositionController.setSetpoint(
        IntakeStateManager.getInstance().getTargetWristState().angle);
    ...
    switch (IntakeStateManager.getInstance().getTargetClawState()) {
      case Idle -> {
        ...
        if (m_intakeBase.m_colorSensorInputs.ProximityValue > 0.6) break;

        IntakeBase.CurrentSeenTarget currentSeenTarget = m_intakeBase.getCurrentSeenTarget();

        if (currentSeenTarget == IntakeBase.CurrentSeenTarget.Unknown) break;

        switch (currentSeenTarget) {
          case Cone -> IntakeStateManager.getInstance()
              .updateState(IntakeStateManager.ValidClawState.HoldingCone);

          case Cube -> IntakeStateManager.getInstance()
      ...
      case HoldingCone -> {
        if (!m_intakeBase.isHoldingCone())
          DriverStation.reportWarning(
              "Claws are set to hold a CONE, but no CONE is detected in the intake", false);

        if (m_intakeBase.isHoldingCube()) {
          DriverStation.reportError(
              "Claws are set to hold a CONE, but a CUBE was detected in the intake, stopping to prevent popping it",
              false);
          break;
        }
      ...
      case HoldingCube -> {
        if (!m_intakeBase.isHoldingCube())
          DriverStation.reportWarning(
              "Claws are set to hold a CUBE, but no CUBE is detected in the intake", false);

        if (m_intakeBase.isHoldingCone())
          DriverStation.reportWarning(
              "Claws are set to hold a CUBE, but a CONE was detected in the intake", false);
    ...
    double wristOutput =
        m_wristPositionController.calculate(m_intakeBase.m_wristEncoderInputs.AbsolutePositionRad);
    double clawOutput =
        m_clawPositionController.calculate(m_intakeBase.m_clawEncoderInputs.AbsolutePositionRad);
